			...
			>
				<Card className="w-[350px] bg-background/60 backdrop-blur-sm">
					<CardHeader className="pb-2">
						...
						<CardDescription>Fetching your workspaces...</CardDescription>
					</CardHeader>
	...
	const handleDeleteSearchSpace = async (id: number) => {
		...
		try {
			...
			throw new Error("Failed to delete workspace");
					...
					// Refresh the workspaces list after successful deletion
		refreshSearchSpaces();
		} catch (error) {
				...
				<div className="flex flex-row space-x-4 justify-between">
					<div className="flex flex-row space-x-4">
						...
						<div className="flex flex-col space-y-2">
							<h1 className="text-4xl font-bold">Strawberries Dashboard</h1>
							<p className="text-muted-foreground">Welcome to your Strawberries dashboard.</p>
						</div>
					</div>
				...
				<div className="flex flex-col space-y-6 mt-6">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						{searchSpaces &&
							...
							searchSpaces.map((workspace: SearchSpace) => (
								<motion.div key={workspace.id} variants={itemVariants} className="aspect-[4/3]">
									<Tilt
										rotationFactor={6}
										isRevese
										springOptions={{
									...
									>
										...
										<div className="flex flex-col h-full overflow-hidden rounded-xl border bg-muted/30 backdrop-blur-sm transition-all hover:border-primary/50">
											<div className="relative h-32 w-full overflow-hidden">
												<Link href={`/dashboard/${workspace.id}/chats`} key={workspace.id}>
													<Image
														...
														<AlertDialog>
															...
															<AlertDialogContent>
																...
																<AlertDialogFooter>
																	...
																	<AlertDialogAction
																		onClick={() => handleDeleteSearchSpace(workspace.id)}
								...
								>
									<Link href="/dashboard/searchspaces" className="flex h-full">
										<div className="flex flex-col items-center justify-center h-full w-full rounded-xl border border-dashed bg-muted/10 hover:border-primary/50 transition-colors">
											...
											<span className="text-sm font-medium">Add New Workspace</span>
										</div>
									</Link>
								</Tilt>
							</motion.div>
